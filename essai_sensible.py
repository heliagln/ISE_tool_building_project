...
def select_closest_sensible_points(df_random, centers, pourcentage=0.05):
    dist_to_centers = pairwise_distances(df_random, centers)
    dist_to_first, dist_to_second = dist_to_centers[:, 0], dist_to_centers[:, 1]
    closest_first = np.argsort(dist_to_first)[:20]
    print(df_random.iloc[closest_first])
    closest_second = np.argsort(dist_to_second)[:20]
    print(df_random.iloc[closest_second])
...
def calculate_idi_ratio_tool(model, X_test, sensitive_columns, non_sensitive_columns, num_samples=1000, num_seed = 100):
  ...
  centers = apply_clustering(pd.DataFrame(df_random['prediction'])) # .drop(columns=non_sensitive_columns))

  f, s = select_closest_sensible_points(pd.DataFrame(df_random['prediction']), centers)# .drop(columns=non_sensitive_columns)
  ...
  first_cluster = (df_random.iloc[f]).reset_index(drop=True)
  print(first_cluster)
  second_cluster = (df_random.iloc[s]).reset_index(drop=True)
  print(second_cluster)
  boundary_samples = (df_random.iloc[boundary_index]).reset_index(drop=True)

  disc = 0
  discrimination_pairs = []
  for index, row in boundary_samples.iterrows():
    sens_1 = first_cluster.iloc[np.random.choice(len(first_cluster))]
    print(sens_1)
    sens_2 = second_cluster.iloc[np.random.choice(len(second_cluster))]
    sample = X_test.iloc[np.random.choice(len(X_test))]
    sample_a = sample.copy()
    print(sample_a)
    sample_b = sample.copy()
    sample_a[sensitive_columns] = sens_1[sensitive_columns].values
    sample_b[sensitive_columns] = sens_2[sensitive_columns].values
    prediction_a = model.predict((np.array(sample_a)).reshape(1, -1))  # Reshape to fit model input format
    prediction_b = model.predict((np.array(sample_b)).reshape(1, -1))
    ...
    if abs(pred_a - pred_b) > 0.05:
        ...
        disc += 1  # Individual discriminatory instance'''
  return disc # discrimination_number / num_samples, discrimination_samples
...
def main():
    ...
    idi_ratio = calculate_idi_ratio_tool(model, X_test, sensitive_columns, non_sensitive_columns, num_samples=500, num_seed=50)
    print(idi_ratio)
