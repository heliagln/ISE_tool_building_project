...
def random_path(model, tree, X_test, sensitive_columns, non_sensitive_columns):
    ...
    borne = pd.DataFrame({
        'max' : [X_test[col].max() for col in columns],# sensitive_columns],
        'min' : [X_test[col].min() for col in columns],# sensitive_columns]
        }, index=columns)# sensitive_columns)
    ...
    non_sensible_nodes = [(columns[tree.tree_.feature[node]], tree.tree_.threshold[node]) for node in node_indices if columns[tree.tree_.feature[node]] not in sensitive_columns]
    df_path = pd.DataFrame(columns=['sample', 'feature_name', 'first_threshold', 'second_threshold'])

    if sensible_nodes != []:
        for feature_name, threshold in non_sensible_nodes:
...
def extract_sample(row):
    borne = row['sample']
    sample_data = [random.uniform(borne.loc[feature_name, 'min'], borne.loc[feature_name, 'max']) for feature_name in borne.index]
    sample = pd.Series(sample_data, index=borne.index)
    row['sample'] = sample
    return row
...
def calculate_idi_ratio_tool(model, X_test, sensitive_columns, non_sensitive_columns, num_samples=1000, num_seed = 100, threshold=0.05, num_training=6000):
    ...
    if sensible_nodes == []: # if there is no disciminating space
        ...
        # extract samples
        df_samples = df_samples.apply(extract_sample, axis=1)
...
def main():
    ...
    IDI_ratio = calculate_idi_ratio_tool(model, X_test, sensitive_columns, non_sensitive_columns, num_samples=1000, num_seed=100, num_training=6000)
